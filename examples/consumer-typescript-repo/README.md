...
```bash
# from this directory. `semctx` here is `bunx @semantic-context/cli` (or the repo's
# `bun apps/cli/src/index.ts`) until the CLI is published to a registry.
bunx @semantic-context/cli init --preset github-claude --dry-run   # preview what would be created
bunx @semantic-context/cli init --preset github-claude             # write config, workflow, .claude note
bunx @semantic-context/cli index                                   # build the deterministic graph
# --base needs the base branch fetched locally (git fetch origin main; CI: actions/checkout fetch-depth: 0)
bunx @semantic-context/cli verify diff --base origin/main          # analyse a range → PASS / WARN / BLOCK
```
